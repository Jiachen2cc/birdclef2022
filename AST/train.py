...
def run_training(model, optimizer, scheduler, device, num_epochs):
    ...
    for epoch in range(1, num_epochs + 1): 
        ...
        if val_epoch_f1 >= best_epoch_f1:
            ...
            best_model_wts = copy.deepcopy(model.state_dict())
            PATH = "trained_model/F1{:.4f}_epoch{:.0f}.bin".format(best_epoch_f1, epoch)
...
if __name__ == '__main__':
    ...
    model = ASTModel(label_dim = 152, input_fdim = 224 ,input_tdim = 313,imagenet_pretrain = True,audioset_pretrain=False)
    model = model.to(device)
    ...
    run = wandb.init(project = CONFIG.competition+'_AST_test',
                    job_type = 'Train',
